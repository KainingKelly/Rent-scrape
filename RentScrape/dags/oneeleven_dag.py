...
from RentScrape.RentScrape.transformers.oneeleven_transform import OneElevenTransform


def dag(crawl: bool, tl: bool):
    #Extract
    if crawl:
        os.system("cd RentScrape; scrapy crawl OneEleven")

    if tl:
        #Transform
        transformer = OneElevenTransform()
        units, floorplans = transformer.grab_files()

        #Load
        result_path = f"{os.getcwd()}/RentScrape/result/oneeleven/{datetime.datetime.now().strftime('%Y-%m-%d')}"
